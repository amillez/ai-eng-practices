...
| **Opus 5.5 xhigh** | Large-work **Orca coordinator** default | **Claude Code** Opus at **xhigh**, driving Orca CLI. Prefer the **Opus 5.5 xhigh** label for orch | No Cursor; coordinator only plans/dispatches/waits (does not integrate/validate) |
| **Fable** | Huge / gnarly escalate (non-orch) | **Claude Code** Fable-class at High (or Opus-equivalent at high effort if Fable unavailable). Escalate effort or replan/split if still thrashing | Escalate **role** for hard single-agent work — **not** the Orca coordinator default (**Opus 5.5 xhigh**) |
...
| Large-work orchestration (needs orch) | **Opus 5.5 xhigh** (Orca coordinator) | **xhigh** | Claude Code + Orca | Prefer **Opus 5.5 xhigh** label. Workers via Orca `--agent claude|codex` + chooser — not all Opus xhigh. Coordinator does not integrate/validate. No Cursor. |
