...
2. **Ensure amillez plugin before coding.** On the **host** (user root), run [`amillez/agent-skills`](https://github.com/amillez/agent-skills) `scripts/ensure-install.sh` (thin alias: `ensure-project.sh`). If the pack is missing, install to `~/.claude` / `~/.agents` / `~/.codex` + user rules + stamp — **not** into the worktree/project tree. If already present, continue — refresh only when policy/skills changed or a human asks (`--force`). Project-specific skills (e.g. `verify-*`) stay in-repo. **Grok Bot dispatch prompts to Claude/Codex must include this host ensure step** first.
