...
2. **Ensure amillez plugin before coding.** On the worktree (or project) path, run [`amillez/agent-skills`](https://github.com/amillez/agent-skills) `scripts/ensure-project.sh`. If the project is new or the pack is missing, install (link skills + rules). If already present, continue — refresh only when policy/skills changed or a human asks. **Grok Bot dispatch prompts to Claude/Codex must include this ensure step** on the target path first. Ensure must leave amillez plugin skills/rules/stamps **gitignored** (machine-local); do **not** commit or PR those artifacts.
